...
function updateArmAngle() {
  ...
  */
  leftWrist = poses[0].keypoints[15];
  leftShoulder = poses[0].keypoints[11];
  leftElbow = poses[0].keypoints[13];
  ...
  if (leftWrist.score > 0.1 && leftElbow.score > 0.1 && leftShoulder.score > 0.1) {
    //console.log(angle);
    elbowAngle = angle;
  }
  else {
...
function inDownPosition() {
  ...
  if (poses[0].keypoints[9].y > poses[0].keypoints[13].y) {
    elbowAboveNose = true;
  }
  else {
